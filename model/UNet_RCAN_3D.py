...
class CNN_block(layers.Layer):
    def __init__(self, filters, kernel):
        ...
        self.conv1 = layers.Conv3D(filters=filters, kernel_size=kernel, padding='same')
        self.conv2 = layers.Conv3D(filters=filters, kernel_size=kernel, padding='same')
        self.ident = layers.Conv3D(filters=filters, kernel_size=1, padding='same')
...
class CAB(layers.Layer):
    def __init__(self, filters_cab, filters, kernel):
        ...
        self.conv1 = layers.Conv3D(filters=filters, kernel_size=kernel, padding='same')
        self.conv2 = layers.Conv3D(filters=filters, kernel_size=kernel, padding='same')
        self.cab_conv = layers.Conv3D(filters=filters_cab, kernel_size=1, padding='same')
        self.conv_cab = layers.Conv3D(filters=filters, kernel_size=1, padding='same')

    def call(self, input_tensor):
        x = self.conv1(input_tensor)
        x = layers.LeakyReLU()(x)
        x = self.conv2(x)
        z = layers.GlobalAveragePooling3D(data_format='channels_last', keepdims=True)(x)
        z = self.cab_conv(z)
...
class RG(layers.Layer):
    def __init__(self, num_cab, filters_cab, filters, kernel):
        ...
        self.conv = layers.Conv3D(filters=filters, kernel_size=kernel, padding="same")
...
class RCAN(layers.Layer):
    def __init__(self, num_RG, num_cab, filters_cab, filters, kernel, dropout):
        ...
        self.conv1 = layers.Conv3D(filters=filters, kernel_size=kernel, padding='same')
        self.conv2 = layers.Conv3D(filters=1, kernel_size=1, padding='same')
...
class UNet(layers.Layer):
    ...
    def call(self, input_tensor):
        ...
        for i in range(len(self.decoder)):
            x = layers.UpSampling3D(2, data_format='channels_last')(x)
